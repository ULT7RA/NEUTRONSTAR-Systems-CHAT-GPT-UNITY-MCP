...
{
    /// <summary>
    /// Manages dynamic port allocation and persistent storage for NEUTRONSTAR Systems CHAT GPT MCP
    /// </summary>
    public static class PortManager
    {
        ...
        /// <summary>
        /// Check if a port is currently being used by NEUTRONSTAR Systems CHAT GPT MCP
        /// This helps avoid unnecessary port changes when Unity itself is using the port
        /// </summary>
        /// <param name="port">Port to check</param>
        /// <returns>True if port appears to be used by NEUTRONSTAR Systems CHAT GPT MCP</returns>
        public static bool IsPortUsedByMCPForUnity(int port)
        {
            try
            {
                // Try to make a quick connection to see if it's an NEUTRONSTAR Systems CHAT GPT MCP server
                using var client = new TcpClient();
                var connectTask = client.ConnectAsync(IPAddress.Loopback, port);
                if (connectTask.Wait(100)) // 100ms timeout
                {
                    // If connection succeeded, it's likely the NEUTRONSTAR Systems CHAT GPT MCP server
