...
namespace MCPForUnity.Editor.Services.AssetGen.Import
{
    ...
    public static class ModelImportPipeline
    {
        ...
        private static AssetGenJob ImportModelFile(AssetGenJob job, string rel, string ext)
        {
            ...
            {
                return Fail(job,
                    "GLB import requires glTFast. Install it from the NEUTRONSTAR Systems CHAT GPT MCP → Dependencies tab, or choose FBX output.");
            }
        ...
        private static AssetGenJob ImportArchive(AssetGenJob job, string zipRel)
        {
            ...
            {
                return Fail(job,
                    "This model is glTF (.glb/.gltf), which requires glTFast. Install it from the NEUTRONSTAR Systems CHAT GPT MCP → Dependencies tab.");
            }
