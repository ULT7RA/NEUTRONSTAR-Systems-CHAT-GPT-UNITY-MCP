...
namespace MCPForUnity.Editor.Services
{
    ...
    public class PackageDeploymentService : IPackageDeploymentService
    {
        ...
        public PackageDeploymentResult DeployFromStoredSource()
        {
            ...
            try
            {
                EditorUtility.DisplayProgressBar("Deploy NEUTRONSTAR Systems CHAT GPT MCP", "Creating backup...", 0.25f);
                string backupPath = CreateBackup(targetPath);

                EditorUtility.DisplayProgressBar("Deploy NEUTRONSTAR Systems CHAT GPT MCP", "Replacing package contents...", 0.7f);
        ...
        public PackageDeploymentResult RestoreLastBackup()
        {
            ...
            try
            {
                EditorUtility.DisplayProgressBar("Restore NEUTRONSTAR Systems CHAT GPT MCP", "Restoring backup...", 0.5f);
